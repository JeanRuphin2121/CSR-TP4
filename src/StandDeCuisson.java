...
public class StandDeCuisson {
    ...
    public void demanderCuisson(Client client) throws InterruptedException {
        synchronized (lock) {
            while (platEnCuisson) {
                lock.wait(); // Attente si un autre plat est en cours de cuisson
            }
            platEnCuisson = true; // Le cuisinier commence la cuisson
            System.out.println("Le cuisinier commence à cuire le plat du client " + client.getId());
        }
        Thread.sleep(1000 + new Random().nextInt(1000)); //Simulation de la cuisson

        synchronized (lock) {
            platEnCuisson = false; // La cuisson est terminée
            System.out.println("Le cuisinier a terminé de cuire le plat du client " + client.getId());
